...
const App: React.FC = () => {
  ...
  const handleUserProfile = async (userId: string, userEmail?: string, showLoading = true) => {
    ...
    try {
      ...
      // 2. SINCRONIZAÇÃO DE ACESSO REMOVIDA A PEDIDO DO USUÁRIO
      // O sistema confia no school_id já gravado no perfil do usuário ('users' table).

      // 2. O acesso agora é validado somente pelo perfil existente
      // SEM SINCRONIZAÇÃO de authorized_access para evitar sobrescrita de dados.
        ...
        if (userData.role === 'school_admin' && userData.school_id) {
          ...
          if (schoolData) {
            ...
            if (!schoolData.active || isOverdue) {
              ...
              // Se tudo ok, busca dados (NON-BLOCKING)
...
const fetchData = async (currentSchoolId: string | null, userId?: string) => {
  ...
  try {
    ...
    // --- SMART RECOVERY (SÊNIOR) ---
    // Se não houver dados, pode ser que o usuário esteja atrelado à escola errada.
    // Vamos verificar se ele é dono de outra escola e trocar automaticamente.
    if (!classesData || classesData.length === 0) {
      console.warn("⚠️ Nenhuma turma encontrada. Tentando Smart Recovery...");

      // A. Tenta achar QUALQUER turma para ver se o ID da escola está errado
      ...
      if (anyClass && anyClass.school_id !== currentSchoolId) {
        // ... Lógica existente de Auto-Repair
      ...
      // B. (NOVO) Se A falhar, busca na tabela 'schools' se eu sou dono de alguma outra escola
      if (userId) { // Só funciona se tivermos o userId
        // Primeiro pegamos o email do usuário atual se não tivermos
        let emailToCheck = userEmail;
        if (!emailToCheck) {
          const { data: u } = await supabase.from('users').select('email').eq('id', userId).single();
          if (u) emailToCheck = u.email;
        }
        ...
        if (emailToCheck) {
          ...
          if (mySchools && mySchools.length > 0) {
            const validSchool = mySchools.find(s => s.id !== currentSchoolId) || mySchools[0];
            if (validSchool && validSchool.id !== currentSchoolId) {
              console.log(`♻️ Smart Recovery: Trocando para escola válida encontrada (${validSchool.id})`);
              setSchoolId(validSchool.id);
              await supabase.from('users').update({ school_id: validSchool.id }).eq('id', userId);

              // Recarrega a página para limpar estados limpo (mais seguro que recursão aqui)
